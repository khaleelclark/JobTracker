...
const inputSchema = z.object({ application_id: z.string().uuid().optional() }).default({});
...
export async function listInterviews(input: unknown) {
  ...
  const interviews = await prisma.interview.findMany({
    where: parsed.data.application_id ? { applicationId: parsed.data.application_id } : undefined,
