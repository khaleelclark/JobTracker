...
test("api CRUD routes cover create/read/update/delete flows", async () => {
  const tempRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "job-tracker-api-crud-"),
  );
  ...
  try {
    ...
    const createApplicationBody = (await createApplicationResponse.json()) as {
    ...
    const listApplicationsBody = (await listApplicationsResponse.json()) as {
      applications: Array<{ id: string }>;
    };
    assert.ok(
      listApplicationsBody.applications.some(item => item.id === applicationId),
    );
    ...
    const createResumeBody = (await createResumeResponse.json()) as {
      resume: { id: string; filePath: string };
    };
    ...
    const listResumesBody = (await listResumesResponse.json()) as {
    ...
    const createMasterResumeBody =
      (await createMasterResumeResponse.json()) as {
    ...
    assert.ok(
      listMasterResumesBody.masterResumes.some(
        item => item.owner === "Patrick",
      ),
    );

    const updateApplicationLinksResponse = await applicationResumesRoute.PATCH(
      buildJsonRequest(
        `http://localhost/api/applications/${applicationId}/resumes`,
        "PATCH",
        {
    ...
    const getApplicationBody = (await getApplicationResponse.json()) as {
      application: { careersPageUrl: string | null };
    };
    assert.equal(
      getApplicationBody.application.careersPageUrl,
    ...
    const createInterviewBody = (await createInterviewResponse.json()) as {
      interview: { id: string };
    };
    ...
    const listInterviewsResponse = await interviewsRoute.GET(
      new Request(
        `http://localhost/api/interviews?applicationId=${applicationId}`,
      ),
    );
    assert.equal(listInterviewsResponse.status, 200);

    const getInterviewResponse = await interviewByIdRoute.GET(
      new Request("http://localhost"),
    ...
    const updateInterviewResponse = await interviewByIdRoute.PATCH(
      buildJsonRequest(
        `http://localhost/api/interviews/${interviewId}`,
        "PATCH",
        {
          applicationId,
          roundIndex: 1,
    ...
    const listReflectionsResponse = await reflectionsRoute.GET(
      new Request(
        `http://localhost/api/reflections?interviewId=${interviewId}`,
      ),
    );
    ...
    const createEmailBody = (await createEmailResponse.json()) as {
      emailLog: { id: string };
    };
    ...
    const createMultiApplicationEmailBody =
      (await createMultiApplicationEmailResponse.json()) as {
        emailLogs: Array<{ applicationId: string }>;
        createdCount: number;
      };
    assert.equal(createMultiApplicationEmailBody.createdCount, 2);
    assert.deepEqual(
      createMultiApplicationEmailBody.emailLogs
        .map(email => email.applicationId)
        .sort(),
    ...
    const createCompanyEmailBody =
      (await createCompanyEmailResponse.json()) as {
    ...
    const listEmailsBody = (await listEmailsResponse.json()) as {
      emailLogs: Array<{ id: string }>;
    };
    assert.ok(listEmailsBody.emailLogs.length >= 2);

    const getEmailResponse = await emailByIdRoute.GET(
      new Request("http://localhost"),
    ...
    const updateEmailToMultipleApplicationsResponse =
      await emailByIdRoute.PATCH(
        buildJsonRequest(`http://localhost/api/emails/${emailId}`, "PATCH", {
    ...
    const createFollowupBody = (await createFollowupResponse.json()) as {
      followup: { id: string };
    };
    ...
    const listFollowupsResponse = await followupsRoute.GET(
      new Request(
        `http://localhost/api/followups?applicationId=${applicationId}`,
      ),
    );
    assert.equal(listFollowupsResponse.status, 200);

    const getFollowupResponse = await followupByIdRoute.GET(
      new Request("http://localhost"),
    ...
    const updateFollowupResponse = await followupByIdRoute.PATCH(
      buildJsonRequest(
        `http://localhost/api/followups/${followupId}`,
        "PATCH",
        {
          applicationId,
          attemptIndex: 2,
    ...
    const updateFollowupResultViaUpsertResponse =
      await followupResultsRoute.POST(
        buildJsonRequest("http://localhost/api/followup-results", "POST", {
          followupAttemptId: followupId,
    ...
    const listFollowupResultsResponse = await followupResultsRoute.GET(
      new Request(
        `http://localhost/api/followup-results?followupAttemptId=${followupId}`,
      ),
    );
    ...
    const listEventsResponse = await engagementEventsRoute.GET(
      new Request(
        `http://localhost/api/engagement-events?applicationId=${applicationId}`,
      ),
    );
    assert.equal(listEventsResponse.status, 200);

    const getEventResponse = await engagementEventByIdRoute.GET(
      new Request("http://localhost"),
    ...
    const updateEventResponse = await engagementEventByIdRoute.PATCH(
      buildJsonRequest(
        `http://localhost/api/engagement-events/${eventId}`,
        "PATCH",
        {
          applicationId,
    ...
    const deleteEventResponse = await engagementEventByIdRoute.DELETE(
      new Request("http://localhost"),
      buildContext(eventId),
    );
    assert.equal(deleteEventResponse.status, 200);

    const deleteFollowupResponse = await followupByIdRoute.DELETE(
      new Request("http://localhost"),
      buildContext(followupId),
    );
    assert.equal(deleteFollowupResponse.status, 200);

    const deleteEmailResponse = await emailByIdRoute.DELETE(
      new Request("http://localhost"),
      buildContext(emailId),
    );
    assert.equal(deleteEmailResponse.status, 200);

    const deleteInterviewResponse = await interviewByIdRoute.DELETE(
      new Request("http://localhost"),
      buildContext(interviewId),
    );
    assert.equal(deleteInterviewResponse.status, 200);

    const deleteResumeResponse = await resumeByIdRoute.DELETE(
      new Request("http://localhost"),
      buildContext(resumeId),
    );
