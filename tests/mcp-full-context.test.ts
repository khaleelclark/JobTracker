...
async function importFullContextTool() {
  const absolute = path.join(
    repoRoot,
    "apps/mcp-server/src/tools/getFullContextDump.ts",
  );
...
async function importGetMasterResumeTool() {
  const absolute = path.join(
    repoRoot,
    "apps/mcp-server/src/tools/getMasterResume.ts",
  );
  const href = `${pathToFileURL(absolute).href}?t=${Date.now()}-${Math.random()}`;
  return import(href) as Promise<{
...
test("mcp full-context dump includes warning, filters, truncation, and stays read-only", async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "job-tracker-mcp-full-context-"),
  );
  ...
  try {
    ...
    for (let index = 0; index < 80; index += 1) {
      ...
      await prisma.application.create({
        data: {
          ...
          appliedAt: new Date(
            `2026-02-${String((index % 28) + 1).padStart(2, "0")}T12:00:00.000Z`,
          ),
        },
      });
    }
    ...
    assert.match(
      String(dump.warning?.message ?? ""),
      /explicitly asked for full\/system-wide context/i,
    );
    ...
    assert.equal(
      dump.meta?.likely_truncated_tables?.includes("applications"),
      true,
    );
    ...
    assert.match(
      String(dump.data?.resumes?.[0]?.content?.text ?? ""),
      /TypeScript/,
    );
    ...
    const mutationPattern =
      /prisma\.[a-zA-Z0-9_]+\.(create|update|upsert|delete|createMany|updateMany|deleteMany)\(/;
...
test("mcp get_master_resume reads owner-specific managed master resume", async () => {
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "job-tracker-mcp-master-resume-"),
  );
  ...
  try {
    ...
    assert.match(String(defaultResult.path ?? ""), /kyle-master-resume\.json$/);
    assert.equal(
      (defaultResult.master_resume as { name?: string }).name,
      "Kyle Clark",
    );
    ...
    assert.match(
      String(patrickResult.path ?? ""),
      /patrick-master-resume\.json$/,
    );
    assert.equal(
      (patrickResult.master_resume as { name?: string }).name,
      "Patrick Michael Tobey",
    );
    ...
    assert.equal(
      (managedResult.master_resume as { name?: string }).name,
      "Alex Resume",
    );
    ...
    assert.equal(
      (wrappedResult.master_resume as { name?: string }).name,
      "Wrapped Resume",
    );
    assert.equal(
      "master_resume" in (wrappedResult.master_resume as object),
      false,
    );
  } finally {
