...
async function generateResumePdf(resume, options = {}) {
...
async function runCli() {
  ...
  if (!inputPath || !outputPdfPath) {
    console.error("Usage: node scripts/generateResumePdf.js <resume.json> <output.pdf> [debug-output.html]");
    process.exitCode = 1;
    return;
  }
  ...
  const result = await generateResumePdf(resume, { outputPdfPath, outputHtmlPath });
  console.log(`PDF written to ${result.pdfPath}`);
  if (result.htmlPath) {
    console.log(`HTML written to ${result.htmlPath}`);
  }
}
